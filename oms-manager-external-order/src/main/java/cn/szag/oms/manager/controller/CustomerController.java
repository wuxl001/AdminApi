...
private KafkaProvider kafkaProvider = new KafkaProvider();
	...
	private final String manager_url = "http://127.0.0.1:8088/oms-manager/customer";
	/**
	 * 新增客户账户信息
	* @Title: add 
	* @Description: TODO 
	...
	* @throws
	...
	public AjaxRes add(HttpServletRequest request,String json){
		...
		try {
			...
			kafkaProvider.sendMessage(TopicEnum.ADD_CUSTOMER.getCode(), new Date().getTime(), TopicEnum.ADD_CUSTOMER.getCode(), json);
	...
	* @throws
	...
	public AjaxRes edit(HttpServletRequest request,String json){
		...
		try {
			...
			kafkaProvider.sendMessage(TopicEnum.EDIT_CUSTOMER.getCode(), new Date().getTime(), TopicEnum.EDIT_CUSTOMER.getCode(), json);

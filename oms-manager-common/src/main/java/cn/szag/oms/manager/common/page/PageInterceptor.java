...
import cn.szag.oms.manager.common.utils.ReflectUtil;
 ...
 * 。在Mybatis中Statement语句是�?�过RoutingStatementHandler对象�?
 * prepare方法生成的�?�所以利用拦截器实现Mybatis分页的一个�?�路就是拦截StatementHandler接口的prepare方法
 * ，然后在拦截器方法中把Sql语句改成对应的分页查询Sql语句，之后再调用
 * StatementHandler对象的prepare方法，即调用invocation.proceed()�?
 * 对于分页而言，在拦截器里面我们还�?要做的一个操作就是统计满足当前条件的记录�?共有多少
 * ，这是�?�过获取到了原始的Sql语句后，把它改为对应的统计语句再利用Mybatis封装好的参数和设
...
public class PageInterceptor implements Interceptor {
	...
	private String getSqlserverPageSql(Page page, StringBuffer sqlBuffer) {
		// 计算第一条记录的位置，Sqlserver中记录的位置是从0�?始的�?

 ...
 * 任务
 * 
 * @ClassName: RuleTaskController
 * @Description: TODO
 * @author dengyanghao
 * @date 2019年12月11日 下午2:34:28
 */
@RestController
@RequestMapping(value = "/ruleTask")
public class RuleTaskController {
	...
	public AjaxRes add(HttpServletRequest request,RuleTask ruleTask) {
		AjaxRes ar = new AjaxRes();
		try {
			String token = request.getParameter("token");
			User user = Verification.getUser(token, redisUtil);
			ruleTaskService.insertSelective(ruleTask);
	...
	public AjaxRes edit(HttpServletRequest request, RuleTask ruleTask) {
		AjaxRes ar = new AjaxRes();
		try {
			String token = request.getParameter("token");
			User user = Verification.getUser(token, redisUtil);
			ruleTaskService.updateByPrimaryKeySelective(ruleTask);
